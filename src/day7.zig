...
test "card less than" {
    ...
    const expected = [_]bool{ false, true, false, true, false, true, false, true, false, true, false, false, false, false };
...
const HandType = enum {
    ...
    fn whichType(hand: []const Card) HandType {
        // var sorted: [5]Card = undefined;
        // @memcpy(&sorted, &hand);
        // std.sort.block(Card, sorted, {}, Card.lessThanSort);
        assert(hand.len == 5);
...
test "hand types" {
    ...
    try testing.expectEqual(.three_of_a_kind, hand4.handtype);

    const hand5 = Hand.init("JJ2AA");
    try testing.expectEqual(.two_pair, hand5.handtype);

    const hand6 = Hand.init("JK77Q");
    try testing.expectEqual(.one_pair, hand6.handtype);
...
test "part 1 example" {
    ...
    const actual: u64 = try part1(input);
